...
if (!self.CASTING_SHELL_LOADED) {
  ...
  (function() {
    var BUILD = 'v68', VERSION = 'casting-' + encodeURIComponent(new URL('./',self.location.href).pathname) + '-' + BUILD;
    ...
    self.addEventListener('activate', function(event) {
      event.waitUntil(caches.keys().then(function(keys) {
        return Promise.all(keys.filter(function(k) { return (k.indexOf('casting-'+encodeURIComponent(BASE.pathname)+'-')===0 || (BASE.pathname==='/MODHAUS-CASTING/' && k.indexOf('casting-v')===0)) && k!==VERSION; })
